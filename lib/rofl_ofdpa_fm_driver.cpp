...
void
rofl_ofdpa_fm_driver::enable_port_pvid_ingress(const std::string &port_name, uint16_t vid)
{
...
void
rofl_ofdpa_fm_driver::enable_port_vid_ingress(const std::string &port_name, uint16_t vid)
{
	assert(vid < 0x1000);
	uint32_t port_no;
...
uint32_t
rofl_ofdpa_fm_driver::enable_port_vid_egress(const std::string &port_name, uint16_t vid, bool untagged)
{
...
uint32_t
rofl_ofdpa_fm_driver::enable_group_l2_multicast(uint16_t vid, uint16_t id,
...
void
rofl_ofdpa_fm_driver::enable_policy_arp(uint16_t vid, uint32_t group_id, bool update)
{
	assert(vid < 0x1000);
...
void
rofl_ofdpa_fm_driver::enable_policy_lldp() {
  rofl::openflow::cofflowmod fm(dpt.get_version());
  fm.set_table_id(OFDPA_FLOW_TABLE_ID_ACL_POLICY);
...
void
rofl_ofdpa_fm_driver::remove_bridging_unicast_vlan(const rofl::cmacaddr& mac,
